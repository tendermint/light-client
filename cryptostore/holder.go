...
func (s Manager) assertSigner() lightclient.KeySigner {
	return s
}
...
// Signature signs data with the private key, and returns in a form that
// can be passed to Verify.
//
// If no key for this name, or the passphrase doesn't match, returns an error
func (s Manager) Signature(name, passphrase string, data []byte) ([]byte, error) {
...
// Verify takes a signature and public key (generated by
// Signature() and Get().PubKey) and make sure they are valid for the given
// data.
//
// Returns an informative error if the signature is invalid, nil if
// the data is properly signed by the given pubkey.
func (s Manager) Verify(data, sigBytes, pubkey []byte) error {
	// TODO: make this more generic, not tied to go-crypto types???
